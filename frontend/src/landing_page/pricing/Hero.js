import React from 'react';

function Hero() {
    return ( 
        <div className="container">
            <div className="row text-center mt-5 p-5 ">
                <h1>Charges</h1>
                <p style={{color:"#9b9b9b"}}>List of all charges and taxes</p>
            </div>
            <div className="row m-5 p-5">
                <div className="col-4 text-center">
                    <img src="media/images/pricing0.svg" style={{width:"70%"}} alt="" />
                    <h2 className='mb-3'>Free equity delivery</h2>
                    <p style={{fontSize:"16px"}}>All equity delivery investments (NSE, BSE), are absolutely free — ₹ 0 brokerage.</p>
                </div>
                <div className="col-4 text-center">
                    <img src="media/images/intradayTrades.svg" style={{width:"70%"}} alt="" />
                    <h2 className='mb-3'>Intraday and F&O trades</h2>
                    <p style={{fontSize:"16px"}}>Flat ₹ 20 or 0.03% (whichever is lower) per executed order on intraday trades across equity, currency, and commodity trades. Flat ₹20 on all option trades.</p>
                </div>
                <div className="col-4 text-center">
                    <img src="media/images/pricing0.svg" style={{width:"70%"}} alt="" />
                    <h2 className='mb-3'>Free direct MF</h2>
                    <p style={{fontSize:"16px"}}>All direct mutual fund investments are absolutely free — ₹ 0 commissions & DP charges.</p>
                </div>
            </div>
        </div>
     );
}

export default Hero;